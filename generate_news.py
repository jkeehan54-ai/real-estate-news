...
────────────────────────────────────────────────────
실행 환경: 파이썬(Python 3.x)
라이브러리: pip install feedparser requests
"""
...
def is_within_24h(entry, now_kst: datetime) -> bool:
    ...
    return (now_kst - pub_dt).total_seconds() <= 86400
...
def get_clean_news() -> dict:
    ...
    for q in queries:
        ...
        try: feed = feedparser.parse(requests.get(url, headers=HEADERS, timeout=8).content)
        except: feed = feedparser.parse(url)
        for entry in feed.entries:
            all_entries.append((get_best_pub_dt(entry), entry))

    all_entries.sort(key=lambda x: x[0] or datetime.max.replace(tzinfo=KST), reverse=True)

    for pub_dt, entry in all_entries:
        if not is_within_24h(entry, now_kst) or is_duplicate(entry.title, seen_raw): continue
        ...
        if len(results[cat]) < 10:
            ...
            url_dt = extract_date_from_url(getattr(entry, 'link', ''))
            pub_str = url_dt.strftime("%m/%d") if url_dt else (pub_dt.strftime("%m/%d %H:%M") if pub_dt else "")
            results[cat].append({"title": normalize(entry.title), "link": entry.link, "src": src, "pub_str": pub_str})
            seen_raw.append(entry.title)
    return results

def build_html(data: dict) -> str:
    today = datetime.now(KST).strftime("%Y년 %m월 %d일")
    style = "<style>body{font-family:'Malgun Gothic',sans-serif;line-height:1.6;max-width:800px;margin:20px auto;padding:20px;}h1{color:#2c3e50;}h2{background:#f8f9fa;padding:10px;border-radius:5px;}a{text-decoration:none;color:#007bff;}</style>"
    html = f"<html><head><meta charset='utf-8'>{style}</head><body><h1>🏠 부동산 뉴스 브리핑 ({today})</h1>\n"
    html += " | ".join(f'<a href="{u}" target="_blank">{n}</a>' for n, u in SOURCES.items())
    html += "\n<h2>오늘의 핵심 브리핑</h2><p>부동산 주요 소식을 실시간으로 정리합니다.</p>"
    for cat, lst in data.items():
        html += f"<h2>[{cat}]</h2>"
        html += "".join(f"<p><a href='{n['link']}' target='_blank'>{n['title']}</a> | {n['src']} <small style='color:#aaa'>({n['pub_str']})</small></p>" for n in lst) if lst else "<p>최근 24시간 내 수집된 기사가 없습니다.</p>"
    return html + "</body></html>"

if __name__ == "__main__":
    output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "index.html")
    data = get_clean_news()
    with open(output_path, "w", encoding="utf-8") as f: f.write(build_html(data))
