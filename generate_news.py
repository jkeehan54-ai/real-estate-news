...
def get_pub_dt(entry):
    """published_parsed(UTC) → KST datetime. 없으면 None."""
...
def is_within_24h(entry, now_kst: datetime) -> bool:
    """
    발행일이 now 기준 24시간 이내이면 True.
    날짜 필드가 없는 기사는 일단 포함(True) 처리.
    """
    pub_dt = get_pub_dt(entry)
    if pub_dt is None:
        return True                          # 날짜 불명 → 포함
    return (now_kst - pub_dt).total_seconds() <= 86400  # 24h = 86400s
...
def get_clean_news() -> dict:
    ...
    seen_raw  = []
    now_kst   = datetime.now(KST)           # 현재 KST 시각
    ...
    for pub_dt, entry in all_entries:
        ...
        if len(results[cat]) < 10:
            ...
            pub_str = pub_dt.strftime("%H:%M") if pub_dt else ""
            results[cat].append({
                "title":  normalize(raw),
                "link":   entry.link,
                "src":    src,
    ...
    print(f"[완료] 24h 필터로 {skipped}건 제외 | 최종 {total_kept}건 수집")
    return results
