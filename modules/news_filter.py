...
def classify(title: str) -> str:
    ...
    # 주택 공급·개발과 직접 연결되는 핵심어
    strong_supply_context = any(
        k in t
        for k in [
            "신도시",
            "공공주택",
    ...
    # 단독으로는 공급개발로 보지 않는 보조 키워드
    weak_supply_context = any(
        k in t
        for k in [
            "역세권",
            "용적률",
    ...
    # 보조 키워드는 주택·건축·개발 문맥이 함께 있을 때만 인정
    housing_development_context = any(
        k in t
        for k in [
            "아파트",
            "주택",
    ...
    if strong_supply_context:
        return "공급개발"

    if weak_supply_context and housing_development_context:
        return "공급개발"
